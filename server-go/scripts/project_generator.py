...
def copy_template_files(template_dir: Path, dest_dir: Path, variables: dict, skip_patterns: list = []):
	"""
	.tmpl 파일: 렌더링 후 .tmpl 제거하고 저장
	일반 파일: 그대로 복사
	skip_patterns: 복사를 건너뛸 파일 패턴 목록 (예: ["Protocol.*.go", "main.*.py.tmpl"])
	"""
	...
	for item in template_dir.rglob("*"):
		...
		for pattern in skip_patterns:
			...
			if fnmatch.fnmatch(item.name, pattern):
				...
				break
			# 경로로 매칭 (.venv/* 등)
			if "/" in pattern and fnmatch.fnmatch(str(rel_path), pattern):
				should_skip = True
				break
			# 디렉토리 경로 매칭 (.venv 등)
			if pattern in str(rel_path).split("/"):
				should_skip = True
				break
...
def create_server_go(output_path: Path, engine_version: str, engine_local: str, template: str = "basic"):
	"""Go 서버 프로젝트 생성 (basic 또는 game 템플릿)"""
	template_label = "Basic (Echo Server)" if template == "basic" else "Simple Game (Number Guessing)"
	print(f"\n🚀 Go 서버 프로젝트 생성: {output_path} [{template_label}]")
	...
	variables = {
		...
		"ENGINE_LOCAL_REPLACE": "\nreplace github.com/zoit/zo-socket-framework/engine/server-go => ./engine/server-go" if engine_local else "",
	}

	# templates/server-go 복사 (템플릿 특정 파일 제외)
	template_dir = get_templates_dir() / "server-go"
	skip_patterns = ["main.*.go.tmpl", "Protocol.*.go"]
	copy_template_files(template_dir, output_path, variables, skip_patterns)

	# 템플릿에 맞는 main.go와 Protocol.go 선택
	main_template = f"main.{template}.go.tmpl"
	protocol_template = f"Protocol.{template}.go"

	# main.go 처리
	main_tmpl_path = template_dir / main_template
	if main_tmpl_path.exists():
		content = main_tmpl_path.read_text(encoding="utf-8")
		rendered = render_template(content, variables)
		main_file = output_path / "main.go"
		main_file.write_text(rendered, encoding="utf-8")
		print(f"  ✓ main.go 생성 ({template} 템플릿)")

	# Protocol.go 처리
	protocol_src = template_dir / "protocol" / protocol_template
	protocol_dst = output_path / "protocol" / "Protocol.go"
	if protocol_src.exists():
		shutil.copy2(protocol_src, protocol_dst)
		print(f"  ✓ Protocol.go 복사 ({template} 템플릿)")
...
def create_client_python(output_path: Path, engine_version: str, engine_local: str, template: str = "basic"):
	"""Python 클라이언트 프로젝트 생성 (basic 또는 game 템플릿)"""
	template_label = "Basic (Echo Client)" if template == "basic" else "Simple Game (Number Guessing)"
	print(f"\n🚀 Python 클라이언트 프로젝트 생성: {output_path} [{template_label}]")
...
zoit-socket-client = {{ path = "{engine_local}", editable = true }}"""
	...
	# templates/client-python 복사 (템플릿 특정 파일 및 .venv 제외)
	template_dir = get_templates_dir() / "client-python"
	skip_patterns = ["main.*.py.tmpl", "Protocol.*.py", ".venv", "__pycache__", ".python-version", "uv.lock"]
	copy_template_files(template_dir, output_path, variables, skip_patterns)

	# 템플릿에 맞는 main.py와 Protocol.py 선택
	main_template = f"main.{template}.py.tmpl"
	protocol_template = f"Protocol.{template}.py"
	...
	if main_tmpl_path.exists():
		...
		main_file.write_text(rendered, encoding="utf-8")
		print(f"  ✓ main.py 생성 ({template} 템플릿)")

	# Protocol.py 처리
	protocol_src = template_dir / "protocol" / protocol_template
	protocol_dst = output_path / "protocol" / "Protocol.py"
	if protocol_src.exists():
		shutil.copy2(protocol_src, protocol_dst)
		print(f"  ✓ Protocol.py 복사 ({template} 템플릿)")

	# engine 복사 (로컬 개발용)
	if engine_local:
		src_engine = Path(engine_local).resolve()
		dst_engine = output_path / "engine" / "client-python"
		if src_engine.exists():
			shutil.copytree(src_engine, dst_engine, dirs_exist_ok=True)
			print(f"  ✓ engine 폴더 복사")
...
def create_client_csharp(output_path: Path, engine_version: str, engine_local: str, template: str = "basic"):
	"""Unity3D C# 클라이언트 프로젝트 생성 (basic 또는 game 템플릿)"""
	template_label = "Basic (Echo Client)" if template == "basic" else "Simple Game (Number Guessing)"
	print(f"\n🚀 C# (Unity3D) 클라이언트 프로젝트 생성: {output_path} [{template_label}]")
	...
	# 템플릿에 맞는 main.cs와 Protocol.cs 선택
	template_dir = get_templates_dir() / "client-csharp"
	main_template = f"main.{template}.cs.tmpl"
	protocol_template = f"Protocol.{template}.cs"

	# main.cs 처리
	main_tmpl_path = template_dir / main_template
	if main_tmpl_path.exists():
		content = main_tmpl_path.read_text(encoding="utf-8")
		variables = {"PROJECT_NAME": output_path.name}
		rendered = render_template(content, variables)
		main_file = output_path / "main.cs"
		main_file.write_text(rendered, encoding="utf-8")
		print(f"  ✓ main.cs 생성 ({template} 템플릿)")

	# Protocol.cs 처리
	protocol_src = template_dir / "Protocol" / protocol_template
	protocol_dst = output_path / "Protocol" / "Protocol.cs"
	if protocol_src.exists():
		protocol_dst.parent.mkdir(parents=True, exist_ok=True)
		shutil.copy2(protocol_src, protocol_dst)
		print(f"  ✓ Protocol.cs 복사 ({template} 템플릿)")
...
def main():
	...
	server_parser.add_argument("--template", choices=["basic", "game"], default="basic", help="템플릿 선택 (기본값: basic)")
	...
	client_parser.add_argument("--template", choices=["basic", "game"], default="basic", help="템플릿 선택 (기본값: basic)")
	...
	try:
		if args.command == "server":
			create_server_go(args.output_path, args.engine_version, args.engine_local, args.template)
		elif args.command == "client":
			if args.language == "python":
				create_client_python(args.output_path, args.engine_version, args.engine_local, args.template)
			elif args.language == "csharp":
				create_client_csharp(args.output_path, args.engine_version, args.engine_local, args.template)
