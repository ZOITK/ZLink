...
type IPacket = base.IPacket""")
        ...
        dispatch_lines.append("// Pack - 메시지 객체를 헤더가 포함된 온전한 패킷 바이트로 변환합니다. (sessionID는 헤더 주입용)")
        ...
        dispatch_lines.append("\t\t\t// UDP 패킷 조립 (sessionID 주입)")
        ...
        dispatch_lines.append("\t\t\t// TCP 패킷 조립 (errorCode는 기본값 Err_None)")
        dispatch_lines.append("\t\t\ttype tcpBuilder interface { BuildTCP(ErrorCode) []byte }")
        dispatch_lines.append("\t\t\tif builder, ok := msg.(tcpBuilder); ok {")
        dispatch_lines.append("\t\t\t\tdata := builder.BuildTCP(Err_None)")
        dispatch_lines.append("\t\t\t\t// TCP 응답 시 서버가 할당한 세션 ID를 헤더에 주입하여 클라이언트에게 알림")
        dispatch_lines.append("\t\t\t\tif len(data) >= base.HeaderSize && sessionID > 0 {")
        dispatch_lines.append("\t\t\t\t\tbinary.LittleEndian.PutUint32(data[14:18], sessionID)")
        dispatch_lines.append("\t\t\t\t}")
        dispatch_lines.append("\t\t\t\treturn data, nil")
        dispatch_lines.append("\t\t\t}")
        dispatch_lines.append("\t\t}")
        dispatch_lines.append("\t}")
        ...
        for pkt in sorted(self.protocol.packets, key=lambda x: x.get_id()):
            ...
            struct_method_lines.append(f"\treturn base.Pack(p.GetID(), body, 0, uint32(errCode), CurrentVersion)")
            struct_method_lines.append("}")
            struct_method_lines.append(f"func (p *Msg_{pkt.name}) BuildUDP(sender uint32) []byte {{")
            struct_method_lines.append("\tbody, _ := p.Encode()")
            struct_method_lines.append(f"\treturn base.Pack(p.GetID(), body, sender, 0, CurrentVersion)")
            struct_method_lines.append("}")
